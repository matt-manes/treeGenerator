...
from printBuddies import printInPlace, clear
...
class TreeGenerator:
    ...
    def _formatBranchName(self, branchName:str, index:int)->str:
        if index == 0:
            ...
            return f'{self._pipe}{("  "+self._pipe)*(index-1)}  {self._tee}{branchName}\n'
    ...
    def _trimTree(self):
        ...
        for i,line in enumerate(tree[:-1]):
            if all(ch in f'{self._pipe} ' for ch in line) and i < len(tree)-2 and self._tee in tree[i+1]:
